...
export const responderMensaje = async (req, res, currentUser) => {
  try {
    const mensaje = req.body;
    const usuario = currentUser.id;
    const mensajesAnteriores = await getMensajeById(mensaje.idMensaje);
    const mensajeAnterior = mensajesAnteriores[0];
    ...
    const idTipoCarpeta = mensajeAnterior.idtipocarpeta;
    const idCategoria = mensajeAnterior.idcategoria;
    const menUsuario = mensajeAnterior.usuario;
    const menIdMensaje = mensajeAnterior.idmensaje;
