...
import {
  handleCreateMensaje,
  responderMensaje,
} from "../services/mensaje.service.js"; // Función para crear mensajes.
import {
  getAllMensajes,
  getMensajeById,
...
import { getUserByUsuario } from "../repositories/user.dao.js";
...
// Controlador para enviar una respuesta a un mensaje
export const sendRespuestaMensajeController = async (req, res) => {
  try {
    const currentUser = req.session.user; // Usuario actual de la sesión.
    const { idmensaje, cuerpomensaje, usuario } = req.body; // Extraer los datos de la petición.

    console.log("Usuario actual:", currentUser);
    console.log("ID del mensaje al que se responde:", idmensaje);
    console.log("Cuerpo del mensaje de respuesta:", cuerpomensaje);

    // Validar que los datos obligatorios estén presentes.
    if (!idmensaje || !cuerpomensaje) {
      return res.status(400).json({ message: "Faltan datos obligatorios" });
    }
    // Obtener el remitente del mensaje original
    const remitente = await getUserByUsuario(usuario);
    if (!remitente) {
      return res.status(404).json({ message: "El remitente del mensaje no fue encontrado" });
    }

    console.log({"Remitente del mensaje original:": remitente, nombre:remitente.nombre, correoalterno:remitente.correoalterno});

    // Llamar a la función que maneja la respuesta del mensaje
    const mensaje = await responderMensaje(idmensaje, cuerpomensaje, currentUser);

    // Agregar al remitente original como destinatario de la respuesta
    /*
    const destinatarios = [remitente.id];
    */
    const destinatario = [{nombreContacto:remitente.nombre, correoContacto:remitente.correoalterno, idTipoCopia: "CO"}]
    ...
    // Responder con éxito si el mensaje se creó correctamente
